...
static qBool R_LoadMD2Model (refModel_t *model)
{
	...
	allocBuffer = static_cast<byte*>(R_ModAlloc (model, sizeof (mAliasModel_t) + sizeof (mAliasMesh_t)));
	...
	outIndex = outMesh->indexes = reinterpret_cast<index_t*>(R_ModAlloc (model, sizeof (index_t) * numIndexes));
	...
	allocBuffer = static_cast<byte*>(R_ModAlloc (model, (sizeof (vec2_t) * numVerts)
...
static qBool R_LoadMD3Model (refModel_t *model)
{
	...
	model->aliasModel = outModel = static_cast<mAliasModel_t*>(R_ModAlloc (model, sizeof (mAliasModel_t)));
	...
	allocBuffer = static_cast<byte*>(R_ModAlloc (model, (sizeof (mAliasFrame_t) * outModel->numFrames)
	...
	for (i=0 ; i<outModel->numMeshes ; i++, outMesh++) {
		...
		// Allocate as much as possible now
#ifdef SHADOW_VOLUMES
		allocBuffer = static_cast<byte*>(R_ModAlloc (model, (sizeof (mAliasSkin_t) * outMesh->numSkins)
...
static qBool R_LoadSP2Model (refModel_t *model)
{
	...
	// Allocate
	//
	allocBuffer = static_cast<byte*>(R_ModAlloc (model, sizeof (mSpriteModel_t) + (sizeof (mSpriteFrame_t) * numFrames)));
...
static qBool SubdivideQ2Polygon (refModel_t *model, mBspSurface_t *surf, int numVerts, float *verts, float subdivideSize)
{
	...
	buffer = static_cast<byte*>(R_ModAlloc (model, sizeof (mQ2BspPoly_t) + ((numVerts+2) * sizeof (bvec4_t)) + ((numVerts+2) * sizeof (vec3_t) * 2) + ((numVerts+2) * sizeof (vec2_t))));
...
static qBool SubdivideQ2BSPLMSurface_r (refModel_t *model, mBspSurface_t *surf, int numVerts, float *verts, float subdivideSize)
{
	...
	// Add a point in the center to help keep warp valid
	buffer = static_cast<byte*>(R_ModAlloc (model, sizeof (mQ2BspPoly_t) + ((numVerts+2) * sizeof (bvec4_t)) + ((numVerts+2) * sizeof (vec3_t) * 2) + ((numVerts+2) * sizeof (vec2_t) * 2)));
	poly = (mQ2BspPoly_t *)buffer;
...
static qBool R_LoadQ2BSPVertexes (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspVertex_t*>(byteBase + lump->fileOfs);
	...
	model->q2BspModel.vertexes = out = reinterpret_cast<mQ2BspVertex_t*>(R_ModAlloc (model, sizeof (*out) * model->q2BspModel.numVertexes));
...
static qBool R_LoadQ2BSPEdges (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspEdge_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->q2BspModel.edges = out = reinterpret_cast<mQ2BspEdge_t*>(R_ModAlloc (model, sizeof (*out) * (model->q2BspModel.numEdges + 1)));
...
{	
	...
	model->q2BspModel.surfEdges = out = static_cast<int*>(R_ModAlloc (model, sizeof (*out) * model->q2BspModel.numSurfEdges));
...
static qBool R_LoadQ2BSPLighting (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	model->q2BspModel.lightData = static_cast<byte*>(R_ModAlloc (model, lump->fileLen));
...
static qBool R_LoadQ2BSPPlanes (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspPlane_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.planes = out = reinterpret_cast<cBspPlane_t*>(R_ModAlloc (model, sizeof (*out) * model->bspModel.numPlanes * 2));
...
static qBool R_LoadQ2BSPTexInfo (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspTexInfo_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->q2BspModel.texInfo = out = reinterpret_cast<mQ2BspTexInfo_t*>(R_ModAlloc (model, sizeof (*out) * model->q2BspModel.numTexInfo));
...
static qBool R_LoadQ2BSPFaces (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspSurface_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.surfaces = out= reinterpret_cast<mBspSurface_t*>(R_ModAlloc (model, sizeof (*out) * model->bspModel.numSurfaces));
...
static qBool R_LoadQ2BSPMarkSurfaces (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<int16*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->q2BspModel.markSurfaces = out = reinterpret_cast<mBspSurface_t**>(R_ModAlloc (model, sizeof (*out) * model->q2BspModel.numMarkSurfaces));
...
static qBool R_LoadQ2BSPVisibility (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	model->q2BspModel.vis = reinterpret_cast<dQ2BspVis_t*>(R_ModAlloc (model, lump->fileLen));
...
static qBool R_LoadQ2BSPLeafs (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspLeaf_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.leafs = out = reinterpret_cast<mBspLeaf_t*>(R_ModAlloc (model, sizeof (*out) * model->bspModel.numLeafs));
	...
	for (i=0 ; i<model->bspModel.numLeafs ; i++, in++, out++) {
		...
		out->q2_firstDecalSurface = reinterpret_cast<mBspSurface_t**>(R_ModAlloc (model, sizeof (mBspSurface_t *) * (numFragSurfaces + 1)));
...
static qBool R_LoadQ2BSPNodes (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspNode_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.nodes = out = reinterpret_cast<mBspNode_t*>(R_ModAlloc (model, sizeof (*out) * model->bspModel.numNodes));
	...
	for (i=0 ; i<model->bspModel.numNodes ; i++, in++, out++) {
		...
		buffer = reinterpret_cast<byte*>(R_ModAlloc (model, size));
...
static qBool R_LoadQ2BSPSubModels (refModel_t *model, byte *byteBase, const dQ2BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ2BspModel_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.subModels = out = reinterpret_cast<mBspHeader_t*>(R_ModAlloc (model, sizeof (*out) * model->bspModel.numSubModels));
...
static qBool R_LoadQ3BSPLighting (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lightLump, const dQ3BspLump_t *gridLump)
{
	...
	if (lightLump->fileLen && !r_vertexLighting->intVal) {
		...
		model->q3BspModel.lightmapRects = reinterpret_cast<mQ3BspLightmapRect_t*>(R_ModAlloc (model, model->q3BspModel.numLightmaps * sizeof (*model->q3BspModel.lightmapRects)));
	}
	...
	inGrid = reinterpret_cast<dQ3BspGridLight_t*>(byteBase + gridLump->fileOfs);
	model->q3BspModel.numLightGridElems = gridLump->fileLen / sizeof (*inGrid);
	model->q3BspModel.lightGrid = reinterpret_cast<mQ3BspGridLight_t*>(R_ModAlloc (model, model->q3BspModel.numLightGridElems * sizeof (*model->q3BspModel.lightGrid)));
...
static qBool R_LoadQ3BSPVisibility (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	model->q3BspModel.vis = reinterpret_cast<dQ3BspVis_t*>(R_ModAlloc (model, lump->fileLen));
...
static qBool R_LoadQ3BSPVertexes (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ3BspVertex_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	buffer = reinterpret_cast<byte*>(R_ModAlloc (model, (count * sizeof (vec3_t) * 2)
...
static qBool R_LoadQ3BSPSubmodels (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ3BspModel_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof(*in)) {
	...
	model->bspModel.subModels = out = reinterpret_cast<mBspHeader_t*>(R_ModAlloc (model, model->bspModel.numSubModels * sizeof (*out)));
	model->bspModel.inlineModels = reinterpret_cast<refModel_s*>(R_ModAlloc (model, sizeof (refModel_t) * model->bspModel.numSubModels));
...
static qBool R_LoadQ3BSPNodes (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ3BspNode_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.numNodes = lump->fileLen / sizeof(*in);
	model->bspModel.nodes = out = reinterpret_cast<mBspNode_t*>(R_ModAlloc (model, model->bspModel.numNodes * sizeof (*out)));
...
static qBool R_LoadQ3BSPFogs (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump, const dQ3BspLump_t *brLump, const dQ3BspLump_t *brSidesLump)
{
	...
	inBrushes = reinterpret_cast<dQ3BspBrush_t*>(byteBase + brLump->fileOfs);
	if (brLump->fileLen % sizeof (*inBrushes)) {
		Com_Printf (PRNT_ERROR, "R_LoadQ3BSPFogs: funny lump size in %s", model->name);
		return false;
	}

	inBrushSides = reinterpret_cast<dQ3BspBrushSide_t*>(byteBase + brSidesLump->fileOfs);
	if (brSidesLump->fileLen % sizeof (*inBrushSides)) {
		Com_Printf (PRNT_ERROR, "R_LoadQ3BSPFogs: funny lump size in %s", model->name);
		return false;
	}

	in = reinterpret_cast<dQ3BspFog_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
		Com_Printf (PRNT_ERROR, "R_LoadQ3BSPFogs: funny lump size in %s", model->name);
		return false;
	}
	...
	model->q3BspModel.fogs = out = reinterpret_cast<mQ3BspFog_t*>(R_ModAlloc (model, model->q3BspModel.numFogs * sizeof (*out)));
	...
	for (i=0 ; i<model->q3BspModel.numFogs ; i++, in++, out++) {
		...
		out->planes = reinterpret_cast<cBspPlane_t*>(R_ModAlloc (model, out->numPlanes * sizeof (cBspPlane_t)));
...
static qBool R_LoadQ3BSPLeafs (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump, const dQ3BspLump_t *msLump)
{
	...
	inMarkSurfaces = reinterpret_cast<int*>(byteBase + msLump->fileOfs);
	...
	in = reinterpret_cast<dQ3BspLeaf_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
		Com_Printf (PRNT_ERROR, "R_LoadQ3BSPLeafs: funny lump size in %s", model->name);
		return false;
	}
	...
	model->bspModel.numLeafs = lump->fileLen / sizeof (*in);
	model->bspModel.leafs = out = reinterpret_cast<mBspLeaf_t*>(R_ModAlloc (model, model->bspModel.numLeafs * sizeof (*out)));
...
static qBool R_LoadQ3BSPEntities (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	out = reinterpret_cast<mQ3BspLight_t*>(R_ModAlloc (model, total * sizeof (*out)));
...
static qBool R_LoadQ3BSPIndexes (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	model->q3BspModel.surfIndexes = out = reinterpret_cast<int*>(R_ModAlloc (model, model->q3BspModel.numSurfIndexes * sizeof (*out)));
...
static qBool R_LoadQ3BSPPlanes (refModel_t *model, byte *byteBase, const dQ3BspLump_t *lump)
{
	...
	in = reinterpret_cast<dQ3BspPlane_t*>(byteBase + lump->fileOfs);
	if (lump->fileLen % sizeof (*in)) {
	...
	model->bspModel.numPlanes = lump->fileLen / sizeof (*in);
	model->bspModel.planes = out = reinterpret_cast<cBspPlane_t*>(R_ModAlloc (model, model->bspModel.numPlanes * sizeof (*out)));
