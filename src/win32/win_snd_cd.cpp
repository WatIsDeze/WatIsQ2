...
static int CDAudio_GetAudioDiskInfo (void)
{
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, (DWORD_PTR)&mciStatusParms);
	if (dwReturn) {
...
void CDAudio_Pause (void)
{
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_PAUSE, 0, (DWORD_PTR)&mciGenericParms);
	if (dwReturn)
...
void CDAudio_Play2 (int track, qBool looping)
{
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_TRACK | MCI_WAIT, (DWORD_PTR)&mciStatusParms);
	if (dwReturn) {
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_PLAY, MCI_NOTIFY | MCI_FROM | MCI_TO, (DWORD_PTR)&mciPlayParms);
	if (dwReturn) {
...
void CDAudio_Stop (void)
{
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_STOP, 0, (DWORD)NULL);
	if (dwReturn)
...
void CDAudio_Resume (void)
{
	...
	mciPlayParms.dwCallback = (DWORD_PTR) sys_winInfo.hWnd;
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_PLAY, MCI_TO | MCI_NOTIFY, (DWORD_PTR)&mciPlayParms);
	if (dwReturn) {
		Com_DevPrintf (PRNT_WARNING, "CDAudio: MCI_PLAY failed (%i)\n", dwReturn);
		return;
	}
		...
		if (cd_isPlaying) {
			...
			if (cd_playLooping) {
				...
				if (++cd_LoopCounter >= cd_loopcount->intVal)
					CDAudio_Play2 (cd_looptrack->intVal, true);
				else
					CDAudio_Play2 (cd_playTrack, true);
			}
		}
		break;
...
qBool CDAudio_Init (void)
{
	...
	dwReturn = (DWORD)mciSendCommand (0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE, (DWORD_PTR)&mciOpenParms);
	if (dwReturn) {
	...
	dwReturn = (DWORD)mciSendCommand (wDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, (DWORD_PTR)&mciSetParms);
	if (dwReturn) {
